...
class Collection {
   ...
   create(doc, docName, cb) {
      ...
      if (!docName || typeof docName !== 'string') {
         const error = new Error("InputError: Document name cannot be empty");
         return cb(error);
      }

      
      const filePath = path.join(this.collectionPath, `${docName}.json`);
      ...
      fs.open(filePath, 'wx', (err, fileDescriptor) => {
         ...
         fs.writeFile(filePath, JSON.stringify(doc), (err) => {
            // Perform error validation
            if (err) {
   ...
   // read a document
   read(docIdentifier, cb) {
      // Perform error handling
      cb = typeof cb === 'function' ? cb : (err, doc) => { };
      if (!docIdentifier) {
         const error = new Error('InputError: Document ID must not be empty');
         return cb(error);
      }

      try {
         const filePath = path.join(this.collectionPath, `${docIdentifier}.json`);
         fs.readFile(filePath, 'utf-8', (err, data) => {
            ...
            if (err) {
               ...
               const error = new Error('Error occured while reading the document');
               return cb(error);
            };
   ...
   listDocsNames(cb) {
      cb = typeof cb === 'function' ? cb : (err, files) => {};

      fs.readdir(this.collectionPath, {withFileTypes: true}, (err, files) => {
         if (err) {
            cb(err);
         };

         // filter only the .json file
         const result = [];
         files.forEach(file => {
            if (!file.isFile() || path.extname(file.name).toLowerCase() !== '.json') return;
            result.push(file.name.replace(path.extname(file.name), ''));
         })
