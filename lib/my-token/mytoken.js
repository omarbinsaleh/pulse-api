...
mytoken.verify = (token, secrete, cb) => {
   ...
   const expectedSignature = crypto.createHmac('sha256', String(secrete)).update(`${header}.${body}`).digest('base64url');
...
// @desc: A function to generate a unique identifier for a token
// @auth: Omar Bin Saleh
mytoken.generateTokenIdentifier = (tokenString) => {
   ...
   const identifier = tokenString.split('.')[2]; // retrieve the signiture part of the token
